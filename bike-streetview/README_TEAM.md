...
現在は、ESP32実機がなくてもキーボード入力と仮想ESP32で主要機能を確認できる状態です。

| 項目 | 状況 |
|---|---|
| キーボード走行 | 実装済み |
| 仮想ESP32 | 実装済み |
| Web Serial接続 | 実装済み・実機検証待ち |
...
1. ESP32実機とのWeb Serial接続テスト
2. エアロバイクへのホールセンサー取り付け
3. GPX・GeoJSONのインポート
4. 作成ルートと走行ログのファイル出力
5. スマートフォンリモコン
6. BLE接続
...
| `firmware/esp32-rpm/` | ESP32ファームウェア |
